...
var S=t=>{let e=h.render(t,{input:g})
return e&&(y=!0,O.insertBefore(e,O.firstChild)),e}
if(h.loading?S("loading"):h.settings.shouldLoad.call(h,g)?0===m.items.length&&S("no_results"):S("not_loading"),(a=h.canCreate(g))&&(d=S("option_create")),h.hasOptions=m.items.length>0||a,y){if(m.items.length>0){if(v||"single"!==h.settings.mode||null==h.items[0]||(v=h.getOption(h.items[0])),!O.contains(v)){let t=0
